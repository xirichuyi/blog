...
use crate::utils::{CompletedVideoPart, R2VideoStorage, VideoMultipartSession};
...
pub async fn begin_video_upload(
    State(storage): State<Arc<R2VideoStorage>>,
    Json(request): Json<BeginVideoUploadRequest>,
) -> Result<Json<ApiResponse<VideoMultipartSession>>> {
    let session = storage
        .begin_upload(&request.file_name, &request.content_type, request.file_size)
        .await?;
