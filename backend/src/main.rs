...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Configure CORS - Log CORS origins for debugging
    tracing::info!("CORS Origins configured: {:?}", config.cors.origins);

    let cors = if cfg!(debug_assertions) || config.cors.origins.is_empty() {
        // Development mode or no CORS origins configured - allow all origins
        tracing::info!("Using permissive CORS (development mode or no origins configured)");
        ...
        tracing::info!("Using specific CORS origins: {:?}", cors_origins);
