...
impl PostRepository {
    ...
    pub async fn list(pool: &DatabasePool, query: PostListQuery) -> Result<(Vec<Post>, i64)> {
        ...
        let posts: Vec<Post> = rows
            ...
            .map(|row| Post {
                ...
                category_name: None, // 普通列表查询不获取分类名，使用list_with_details获取完整信息
    ...
    pub async fn list_with_complete_info(
        ...
        let mut posts_with_complete_info = Vec::new();
    ...
    pub async fn update(
        ...
        Ok(Some(Post {
            ...
            cover_url: row.cover_url,
            content: row.content,
            category_name: None, // 更新时不重新获取分类名
            category_id: row.category_id,
            status: row.status as i32,
            post_images: row.post_images,
