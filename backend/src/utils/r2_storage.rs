...
impl R2Storage {
    ...
    pub async fn begin_image_upload(
        &self,
        file_name: &str,
        content_type: &str,
        file_size: u64,
    ) -> Result<UploadSession> {
        validate_image(file_name, content_type, file_size)?;
        let key = image_key(file_name)?;
        let client = self.client()?;
        Ok(UploadSession {
            mode: "single",
            upload_id: None,
            public_url: format!("{}/{}", client.public_url, key),
            upload_url: Some(presign_put_object(client, &key, content_type, Utc::now())),
            key,
            part_size: None,
            parts: Vec::new(),
        })
    ...
    pub async fn delete_object(&self, key: &str) -> Result<()> {
        ...
        let response = client
            .http
            .delete(format!("{}{}", client.endpoint, path))
    ...
    /// origin. Historical local or third-party URLs are intentionally ignored.
    pub async fn delete_public_url(&self, url: &str) -> Result<()> {
        let client = self.client()?;
        let prefix = format!("{}/", client.public_url);
...
fn validate_image(file_name: &str, content_type: &str, file_size: u64) -> Result<()> {
    const MAX_IMAGE_SIZE: u64 = 10 * 1024 * 1024;
    if file_size == 0 || file_size > MAX_IMAGE_SIZE {
        return Err(AppError::BadRequest(
            "Image must be between 1 byte and 10 MiB".to_string(),
        ));
    }
    ...
    if content_type != expected {
        return Err(AppError::BadRequest(
            "The image extension and content type do not match".to_string(),
        ));
    }
    Ok(())
}
...
fn image_extension(file_name: &str) -> Result<String> {
    let extension = Path::new(file_name)
        ...
        .ok_or_else(|| AppError::BadRequest("Image file has no extension".to_string()))?;
    match extension.as_str() {
        "jpg" | "jpeg" | "png" | "gif" | "webp" => Ok(extension),
...
fn image_key(file_name: &str) -> Result<String> {
    Ok(format!(
        "images/{}/{}.{}",
        Utc::now().format("%Y/%m"),
        Uuid::new_v4(),
...
fn validate_upload_reference(key: &str, upload_id: &str) -> Result<()> {
    validate_object_key(key)?;
    if !(key.starts_with("videos/") || key.starts_with("books/")) {
...
fn validate_object_key(key: &str) -> Result<()> {
    ...
    if is_safe {
        ...
        Err(AppError::BadRequest("Invalid R2 object key".to_string()))
    }
}
...
fn completion_xml(parts: &[CompletedUploadPart]) -> String {
    let body = parts
        .iter()
        .map(|part| {
            format!(
...
mod tests {
    ...
    async fn begins_multipart_upload_and_presigns_every_part() {
        ...
        assert_eq!(session.mode, "multipart");
        assert_eq!(session.upload_id.as_deref(), Some("upload-123"));
        assert_eq!(session.part_size, Some(TARGET_PART_SIZE));
    ...
    #[tokio::test]
    async fn image_upload_is_a_short_lived_direct_put() {
        let storage = R2Storage::new(&test_config(
            "https://account.r2.cloudflarestorage.com".to_string(),
        ));
        let session = storage
            .begin_image_upload("cover.png", "image/png", 1024)
            .await
            .expect("image upload session");
        assert_eq!(session.mode, "single");
        assert!(session.key.starts_with("images/"));
        assert!(session.upload_id.is_none());
        let url = session.upload_url.expect("signed PUT URL");
        assert!(url.contains("X-Amz-Expires=900"));
    ...
    fn rejects_unsupported_files_and_unsafe_completion_data() {
        assert!(validate_image("archive.zip", "application/zip", 10).is_err());
        assert!(validate_image("cover.png", "image/png", 10).is_ok());
